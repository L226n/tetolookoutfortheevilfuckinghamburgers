def __main__():
                ...
                for burger in menu.menu:
                    if friend.rect.colliderect(burger.rect):
                        if burger.good:
                            ...
                            if maxteto < len(self.friends): maxteto = len(self.friends)
                            return "WAHOO"
                        else:
    ...
    joker = pygame.font.Font("Jokerman.otf", size=30)
    urname = joker.render("enter ur name here:;", 0, [255, 0, 255])
    userstring = ""
    ...
    congrats = rescale(image_load("congrats.jpg").convert_alpha(), 0.8)
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if bogus != 50:
                  if event.key == pygame.K_p and not bogus:
                      paused = not paused
                  if event.key == pygame.K_r and not bogus:
                        del friendlies, ground, menu, cream, idkwhattocallthisclass
                        friendlies = friends(tondella)
                        ground = grounds()
                        menu = burger_menu()
                        cream = screamersss()
        ...
        if v and not mousetoggle and not bogus and not paused:
            mousetoggle = True
        ...
        if not paused:
            ...
            if lobster == "ARGH":
                ...
                if len(friendlies.friends) == 0:
                    congregation = joker.render("congrats you scored " + str(maxteto), 0, [255, 255, 0])
                    bogus = 1
                screameraudio.play()
            elif lobster == "WAHOO":
                friendlies.tetotext = friendlies.font.render(str(len(friendlies.friends)), 0, [0, 0, 0])
            ...
            cream.show()
            if bogus == 50:
                display.blit(congrats, (75, 150))
                display.blit(congregation, (350, 150))
                display.blit(urname, (351, 190))
                display.blit(joker.render(userstring, 0, [0, 255, 255]), (350, 230))
            #updateado
            pygame.display.flip()
        clock.tick(30)
